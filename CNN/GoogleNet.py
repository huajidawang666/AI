...
class AuxiliaryClassifier(nn.Module):
    def __init__(self, in_channels, num_classes):
        super().__init__()
        # assume input as 14x14
        self.avg_pool = nn.AvgPool2d(kernel_size=5, stride=3)                           # ix14x14 -> ix4x4
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, 128, kernel_size=1), nn.ReLU(inplace=True),          # ix4x4 -> 128x4x4
        )
        self.flatten = nn.Flatten()                                                     # 128x4x4 -> 2048
        self.fc1 = nn.Sequential(
            nn.Linear(128*4*4, 1024), nn.ReLU(inplace=True), nn.Dropout(AUX_DROPOUT)    # 2048 -> 1024
        )
...
class GoogleNet(nn.Module):
    ...
    def __init__(self, num_class=NUM_CLASSES, use_auxiliary=True):
        ...
        # assume input as 224x224
        self.before_inception = self._get_before_inception()                    # 3x224x224 -> 192x28x28
        self.inception_3a = InceptionBlock(192, 64, 96, 128, 16, 32, 32)        # 192x28x28 -> 256x28x28
        self.inception_3b = InceptionBlock(256, 128, 128, 192, 32, 96, 64)      # 258x28x28 -> 480x28x28
        self.max_pool_2 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)      # 480x28x28 -> 480x14x14
        
        self.inception_4a = InceptionBlock(480, 192, 96, 208, 16, 48, 64)       # 480x14x14 -> 512x14z14
        self.inception_4b = InceptionBlock(512, 160, 112, 224, 24, 64, 64)      # 512x14x14 -> 512x14x14
        self.inception_4c = InceptionBlock(512, 128, 128, 256, 24, 64, 64)      # 512x14x14 -> 512x14x14
        self.inception_4d = InceptionBlock(512, 112, 144, 288, 32, 64, 64)      # 512x14x14 -> 528x14x14
        self.inception_4e = InceptionBlock(528, 256, 160, 320, 32, 128, 128)    # 528x14x14 -> 832x14x14
        self.max_pool_3 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)      # 832x14x14 -> 832x7x7
        
        self.inception_5a = InceptionBlock(832, 256, 160, 320, 32, 128, 128)    # 832x7x7 -> 832x7x7
        self.inception_5b = InceptionBlock(832, 384, 192, 384, 48, 128, 128)    # 832x7x7 -> 1024x7x7
        self.final_avg_pool = nn.AdaptiveAvgPool2d((1, 1))                      # 1024x7x7 -> 1024x1x1
    ...
    def _get_before_inception(self):
        return nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3), nn.ReLU(inplace=True), # 224x224 -> 112x112
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1), # 112x112 -> 56x56
            nn.BatchNorm2d(64),
            
            nn.Conv2d(64, 64, kernel_size=1), nn.ReLU(inplace=True), # 56x56 -> 56x56
            nn.Conv2d(64, 192, kernel_size=3, padding=1), nn.ReLU(inplace=True), # 56x56 -> 56x56
            nn.BatchNorm2d(192),
