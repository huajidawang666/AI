...
from CNN.NUNet import NestedUNet, train_transforms
...
with torch.no_grad():
    ...
    pred_mask = F.one_hot(indices, num_classes=3)
    cv2.imwrite(str(config.DATA_DIR / 'TCGA-18-5592-01Z-00-DX1_0_0_pred.png'), pred_mask.squeeze(0).cpu().numpy().astype('uint8') * 255)