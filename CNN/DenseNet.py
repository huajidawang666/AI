...
class DenseNet(nn.Module):
    ...
    def __init__(self, num_classes=NUM_CLASSES):
        super().__init__()
        # stem
        # assume input as 3x224x224
        self.stem = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False), # 64x112x112
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1) # 64x56x56
        )
        self.dense_block1 = DenseBlock(num_layers=6, in_channels=64, growth_rate=32) # 256x56x56
        self.transition1 = TransitionLayer(in_channels=256, out_channels=128) # 128x28x28
        self.dense_block2 = DenseBlock(num_layers=12, in_channels=128, growth_rate=32) # 512x28x28
        self.transition2 = TransitionLayer(in_channels=512, out_channels=256) # 256x14x14
        self.dense_block3 = DenseBlock(num_layers=24, in_channels=256, growth_rate=32) # 1024x14x14
        self.transition3 = TransitionLayer(in_channels=1024, out_channels=512) # 512x7x7
        self.dense_block4 = DenseBlock(num_layers=16, in_channels=512, growth_rate=32) # 1024x7x7
