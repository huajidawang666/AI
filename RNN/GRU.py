...
class GRULayer(nn.Module):
    def __init__(self, emb_dim, hid_dim):
        ...
        self.W_Gx = nn.Linear(emb_dim, 2 * hid_dim)  # 输入到门的权重
        self.W_Gh = nn.Linear(hid_dim, 2 * hid_dim)  # 隐藏状态到门的权重
        self.W_x = nn.Linear(emb_dim, hid_dim)  # 输入到候选隐藏状态的权重
        self.W_h = nn.Linear(hid_dim, hid_dim)  # 隐藏状态到候选隐藏状态的权重
        ...
        with torch.no_grad():
            ...
            self.W_Gx.bias.fill_(1.0)
            self.W_Gh.bias.fill_(1.0)
    ...
    def forward(self, x, hidden=None):
        ...
        for t in range(seq_len):
            gates = self.W_Gx(x[:, t, :]) + self.W_Gh(hidden)  # (batch_size, 2*hid_dim)
            z, r = gates.chunk(2, dim=1)  # z: Update Gate
                                          # r: Reset Gate
            z = self.sigmoid(z)
            r = self.sigmoid(r)
            h_bar = self.tanh(self.W_x(x[:, t, :]) + self.W_h(r * hidden))  # 候选隐藏状态
            hidden = (1 - z) * hidden + z * h_bar
...
class SimpleGRU(nn.Module):
    def __init__(self, vocab_size, emb_dim, hid_dim):
        ...
        self.rnn = GRULayer(emb_dim, hid_dim)
...
model = SimpleGRU(VOCAB_SIZE, 100, 128).to(DEVICE)
