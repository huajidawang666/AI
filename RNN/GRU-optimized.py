...
class simpleGRU(nn.Module):
    def __init__(self, vocab_size, emb_dim, hid_dim, dropout=0.5):
        ...
        # 在 GRU 层内添加 dropout（仅对多层有效，单层需手动在输出加）
        self.gru = nn.GRU(emb_dim, hid_dim, batch_first=True, num_layers=2, dropout=dropout)
        self.dropout = nn.Dropout(dropout)
        self.fc = nn.Linear(hid_dim, 1)
        
    def forward(self, x):
        x = self.dropout(self.embedding(x)) # Embedding 后的 Dropout
        _, hidden = self.gru(x)
        # 取最后一层 GRU 的隐藏状态
...
model = simpleGRU(VOCAB_SIZE, 100, 64).to(DEVICE)
